...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/nft/collections", async (req, res) => {
    try {
      ...
      if (error.name === "ZodError") {
        res.status(400).json({ error: fromZodError(error).toString() });
      } else if (error.code === "23505" && error.constraint === "nft_collections_contract_address_unique") {
        res.status(400).json({ 
          error: "This contract address is already registered. Please use a different contract address." 
        });
      } else {
